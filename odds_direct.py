...
# Step 3: For each event, get odds with bookmakers parameter
all_odds = []
for idx, ev in enumerate(events[:20]):  # Limit to 20 for testing
    event_id = ev.get("id")
    if not event_id:
        continue
    odds_url = f"https://api.odds-api.io/v3/odds?apiKey={API_KEY}&eventId={event_id}&bookmakers=bet365"
    try:
        r = requests.get(odds_url, timeout=10)
        if r.status_code == 200:
            odds_data = r.json()
            all_odds.append((ev, odds_data))
            logger.info(f"Got odds for {event_id} ({idx+1}/{min(len(events),20)})")
        else:
