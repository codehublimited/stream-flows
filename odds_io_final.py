...
PG_PASSWORD = 'if 'soccer' in name or 'football' in name:'
...
for sport in sports:
    name = sport.get('name', '').lower()
        ...
        break
if not football:
    logger.error('No football/soccer sport found. Available:')
    for s in sports:
        logger.info(f"  {s.get('id')} – {s.get('name')} (slug: {s.get('slug')})")
    exit()

sport_slug = football['slug']
logger.info(f"Using: {football['name']} (slug: {sport_slug})")
...
for idx, ev in enumerate(events):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            logger.info(f'Got odds for {event_id} ({idx+1}/{len(events)})')
        else:
            logger.warning(f'Odds for {event_id} failed: {r.status_code}')
