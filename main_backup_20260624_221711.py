...
async def get_bond_matches(bond_id: int, user_id: int):
    ...
    if not row:
        ...
        conn.close()
        raise HTTPException(status_code=404, detail="Bond not found")
    match_ids, predictions = row  # predictions is already a dict from psycopg2
    # Convert match_ids to text for ANY query
    match_ids_text = [str(mid) for mid in match_ids]
    # Use correct column names: home_team_id, away_team_id, match_date (if that's the column)
    cur.execute("""
        SELECT 
            m.match_id,
    ...
    result = []
    for m in matches:
        match_id, home_team, away_team, match_date, status, score_home, score_away = m
