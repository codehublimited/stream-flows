...
# Try to import telegram_auth; if not found, we'll skip it
try:
    from telegram_auth import verify_telegram_init_data
except ImportError:
    verify_telegram_init_data = None
...
async def telegram_auth(req: AuthRequest):
    if verify_telegram_init_data is None:
        raise HTTPException(status_code=501, detail="Telegram auth not configured")
    is_valid, user_data = verify_telegram_init_data(req.init_data)
    if not is_valid:
...
async def get_bond_matches(bond_id: int, user_id: int):
    ...
    if not row:
        ...
        conn.close()
        raise HTTPException(status_code=404, detail="Bond not found")
    match_ids, predictions_json = row
    predictions = json.loads(predictions_json)
    cur.execute("""
        SELECT match_id, home_team, away_team, match_date, status, score_home, score_away
        FROM matches
